...
text_tokenizer = RobertaTokenizer.from_pretrained('roberta-base', do_lower_case=True)
image_processor = VivitImageProcessor.from_pretrained("google/vivit-b-16x2-kinetics400")

def collate_fn(batch):
    audio_inputs, text_inputs, video_inputs, labels = zip(*batch)

    audio_inputs = audio_processor(audio_inputs, return_tensors='pt', sampling_rate=16000, max_length=16000 * 6, truncation=True, padding=True)
    text_inputs = text_tokenizer(text_inputs, return_tensors='pt', padding='max_length', max_length=128, truncation=True)
    video_inputs = image_processor(video_inputs, return_tensors="pt")
